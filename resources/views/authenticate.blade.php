...
@section('template_title', __('webauthn::messages.auth.title'))
...
@section('content')
    ...
    <form method="POST" action="{{ route('webauthn.auth') }}" id="form">
        ...
        <input type="hidden" name="webauthn_identifier" id="webauthn_identifier" value="{{ $user->getWebauthnIdentifier() }}">
        <input type="hidden" name="once" id="once" value="{{ $once }}">
    </form>
